...
def forwardCheck(depth,reviseFunction,dom,queue,arcs,constraints,heuristic,hType):
    ...
    # Apply the heuristic that we're using at the appropriate depth
    if hType=='dynamic':
        queue = heuristic(queue,dom,depth)
    elif hType=='static' and depth==0:
        queue = heuristic(queue,constraints)
    
    # Remove one value from the domain at this depth level
    value = dom[queue[depth]].pop(0)
    
    # Store a backup of the other possible values
    leftOver = dom[queue[depth]]
    
    # Change the domain of the variable at this depth to the single value
    dom[queue[depth]] = [value]
    consistent = True
    
    # Check for arc consistency in all future variables
    for arc in arcs:
        if queue[depth]==arc[1] and consistent:
            consistent = reviseFunction(arc,dom)
    
    # If all OK, then proceed to the next depth, or return as True
    if consistent:
    ...
    # If not, and if there are some other variables to try, re-assign the domain of the variable to all other options and call forwardCheck again at the same depth
    elif len(leftOver)>0:
            dom[queue[depth]]=leftOver
            return forwardCheck(depth,reviseFunction,dom,queue,arcs,constraints,heuristic,hType)
            
    # If there are no more possibilities, then there is no solution
    else:
        return False
...
def main():
    ...
    for h in (0,1,2):
        ...
        # Perform 1000 trials, and add the time taken each time to a list
        for i in range(0,1000):
            
            # Initialist the problem
            ...
            # Calculate the time taken for the function to execute, and append to the list
            wrapped = wrapper(forwardCheck,0,reviseInequality,dom,queue,arcs,constraints,heuristics[h],hType)
            timeTaken.append(timeit.timeit(wrapped, number=1))
        
        # Average the time of the trials, and print
        print( 'Average time taken using %s heuristic is %f' % (str(hNames[h]), sum(timeTaken)/len(timeTaken)) )
