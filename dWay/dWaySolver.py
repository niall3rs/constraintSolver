...
def forwardCheck(depth,reviseFunction,dom,queue,arcs,constraints,heuristic,hType):
    ...
    # Save a copy of the dom for backup (copying)
    domBackup = copy.deepcopy(dom)
    
    for value in dom[queue[depth]][:]:
        # Reduce the domain to one element
        dom[queue[depth]] = [value]
        consistent = True
        
        # Check all future arcs for consistency
        for arc in arcs:
            if queue[depth]==arc[1] and consistent:
                consistent = reviseFunction(arc,dom)
        
        # If consistent, recursively call or return at the final depth
        if consistent:
        ...
        # If not consistent, restore dom and try another value at this depth        
        dom = copy.deepcopy(domBackup)
